...
void streamSetTimeout(struct Stream *stream, unsigned long timeout)  // sets the maximum number of milliseconds to wait
{
  stream ->_timeout = timeout;
}

unsigned long streamGetTimeout(struct Stream *stream)
{
  return stream ->_timeout;
}
...
bool streamFindUntilLength(struct Stream *stream, char *target, size_t targetLen, char *terminator, size_t termLen)
{
  if (terminator == NULL) {
...
//
size_t streamReadBytes(struct Stream *stream, char *buffer, size_t length)
{
  size_t count = 0;
  while (count < length) {
    int c = timedRead();
...
size_t streamReadBytesUntil(struct Stream *stream, char terminator, char *buffer, size_t length)
{
